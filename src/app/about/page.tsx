            ...
            {experiences.map((exp, i) => (
              <div key={i}>
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-3">
                  <div>
                    <h3 className="text-lg font-semibold flex items-center gap-2" style={{ color: "var(--foreground)" }}>
                      {exp.company}
                      <a href={exp.link} target="_blank" rel="noopener noreferrer" style={{ color: "var(--muted)" }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-sm" style={{ color: "var(--muted)" }}>{exp.role}</span>
                      <span className="px-2 py-0.5 text-xs font-medium" style={{ border: "1px dashed var(--dotted-border)", color: "var(--accent)" }}>
                        {exp.type}
                      </span>
                    </div>
                  </div>
                  <span className="text-sm flex items-center gap-1.5 shrink-0" style={{ color: "var(--muted)" }}>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-4">
                  <span className="text-sm font-medium" style={{ color: "var(--foreground)" }}>Tech Stack:</span>
                  {exp.stack.map((tech) => (
                    <span key={tech} className="px-2 py-0.5 text-xs font-medium" style={{ backgroundColor: "var(--tag-bg)", color: "var(--tag-text)" }}>
                      {tech}
                    </span>
                  ))}
                </div>
